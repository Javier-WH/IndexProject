...
export function DataName() {
    ...
    const [studentName, setStudentName] = useState(`${selectedStudent.names} ${selectedStudent.lastNames}`);
    const [studentCI, setStudentCI] = useState(selectedStudent.ci);
    ...
    useEffect(()=>{
        ...
        try {
            ...
            setModal({state:true, title:"Ha ocurrido un error", message:"El estudiante no tiene notas asignadas a esta materia", type: "warning", buttons: 1})
            setLap1(0);
            setLap2(0);
            setLap3(0);
            setDef(0);
